...
class Server {
  constructor(compiler, options = {}, _log) {
    ...
    // 1. 将传进来的参数挂载在 server 实例上
    this.compiler = compiler;
    this.options = options;

    this.log = _log || createLogger(options);

    // transportMode 会提示是实验功能
    ...
    // 规格化参数，改变 this.options 本身
    normalizeOptions(this.compiler, this.options);

    // updateCompiler 方法是在 compiler.options 上的 entry、plugins 做操作
    // entry 上加上 websocket 和 hot update（有 hot, hotOnly 配置时才这样做） 的 JS 脚本
    // plugins 上新增全局模块 Plugin 和 HotModuleReplacementPlugin（有 hot，hotOnly 时且 compiler 没有这个插件时）
    ...
    // 下面一大段都是把 options 的属性挂载到 this 上面
    this.originalStats =
    ...
    // 后续版本不再把 options 上的属性挂载到 this 上
    ...
    // 注册监听事件
    // 注册编译失败、编译成功时的钩子函数，负责发送信息给客户端
    this.setupHooks();
    // 生成一个 express 实例挂载在 this.app 上
    this.setupApp();
    // express 服务器所有路由都会检查 headers
    this.setupCheckHostRoute();
    // 生成 webpack-dev-middleware 实例
  ...
  setupHooks() {
    ...
    const addHooks = (compiler) => {
      ...
      // 一个新的编译 (compilation) 创建之后，钩入 (hook into) compiler 时触发
      compile.tap('webpack-dev-server', invalidPlugin);
      // 监听模式下，编译无效时触发
      invalid.tap('webpack-dev-server', invalidPlugin);
      // 编译 (compilation) 完成时触发
    ...
    // 多个 compilers 则每个都应用 addHooks 函数，一个的话就直接调用 addHooks 函数
  ...
  // express 服务器所有路由都会检查 headers，检查不通过会返回信息给客户端
  // 【Todo】具体检查啥还不知道
  setupCheckHostRoute() {
